...
from .kronecker_ops import kvs_dot_vec_sparse, kvs_dot_vec, kron_vec_apply, kvs_dot_mat, kron_mat_apply, kron
...
class VGP_kron(gpflow.models.GPModel, gpflow.models.InternalDataTrainingLossMixin):
    def __init__(self, data, ms, a, b, kernels, likelihood, use_two_krons=False, use_extra_ranks=0, d=0, T=0, sparse=False):
        """
        Here we assume the interval is [a,b]
        """
        X, Y = data
    ...
    def _predict_f(self, X, Kuf, full_cov):
        ...
        if(self.sparse):
            mu = kvs_dot_vec_sparse(KfuKi, self.q_mu, self.d, self.T)
        else:
        ...
        if full_cov:
            ...
            if self.use_two_krons:
                ...
                for i in range(self.use_extra_ranks):
                    if(self.sparse):
                        tmp = kvs_dot_vec_sparse(KfuKi, self.q_sqrt_W[:, i : i + 1], self.d, self.T)
                    else:
