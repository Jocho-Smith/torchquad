...
def kvs_dot_vec_specialfirst(k, c, d=0, T=0, sparse = False):
    """
    kvs is a kron-vec stack. A matrix where each row is a kronecker product of
    ...
    #folgende beiden Zeilen sind gegenüber dem Originalcode etwas vereinfacht. Die nachfolgende Zeile ist gestrichen, weil sie überflüssig war.
    C = tf.reshape(c, tf.stack([D0, -1])) 
    C = tf.matmul(k[0],C)   # XXX
...
def kvs_dot_vec_sparse(k, c, d, T):
    """
    kvs is a kron-vec stack. A matrix where each row is a kronecker product of
    row-vectors.  This implementation requires less memory than the memhungry
    version (I hope), and is much faster than the looped version.
    ...
    for ks in k:
        D = D+1           
    M = int(round(((c.shape[0])**(1./D)-1)/2))

    #Berechnung des hyperbolischen Kreuzes. Eine deutliche Beschleunigung dieser Berechnung wäre möglich, indem 1. die Symmetrie ausgenutzt würde und 2. die Tatsache, dass das hyperbolische Kreuz eine klare Begrenzung besitzt und man sich für alle Elemente außerhalb dieser Grenze die Berechnung sparen kann.
    hypcross =  np.array([   np.prod(1 + np.array(i) % (M+1) + (np.array(i) >= M+1))  *
                          ( (1 + np.max( np.array(i) % (M+1) + (np.array(i) >= M+1) )) **(-T) )  
                          <= (1+d)**(1-T)
            for (i) in np.ndindex(tuple([2*M+1]*D)) ])
    
    
    #Alle Elemente von C außerhalb des hyperbolischen Kreuzes sind nun 0.
    C = tf.math.multiply(
        tf.convert_to_tensor(np.array([hypcross]), dtype=tf.float64),
        tf.reshape(c, [1,-1]) )
